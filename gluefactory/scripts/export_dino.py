...
from ..settings import DATA_PATH, DATA_DINO_PATH
from ..utils.export_predictions import export_predictions, export_seg_homo_predictions
...
configs = {
    ...
    "dino": {
        "name": f"r{resize}_dino",
        "keys": ["features"],
        "gray": False,
        "conf":  {
            "name": "backbones.dinov2_pipeline",
            "allow_resize": True,  
        },
    },
    "sift": {
...
def run_export(feature_file, scene, args):
    ...
    #     callback_fn = None
    export_predictions(
        loader, model, feature_file, as_half=True, keys=keys, callback_fn=None#callback_fn
    )
...
if __name__ == "__main__":
    ...
    export_root = Path(DATA_DINO_PATH, "exports", "megadepth-undist-depth-" + export_name)
