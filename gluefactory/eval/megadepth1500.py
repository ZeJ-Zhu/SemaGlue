...
from .utils import eval_matches_epipolar, eval_poses, eval_relative_pose_robust, eval_homography_dlt
...
class MegaDepth1500Pipeline(EvalPipeline):
    ...
    def _init(self, conf):
        if not (Path(DATA_PATH) / "megadepth1500").exists():
            ...
            zip_path.unlink()
        else: logger.info("MegaDepth-1500 数据集已存在，无需下载。")
    ...
    def get_predictions(self, experiment_dir, model=None, overwrite=False):
        ...
        if not pred_file.exists() or overwrite:
            if model is None:
                #model_path="/home/lpw/.cache/torch/hub/checkpoints/superpoint_lightglue_v0-1_arxiv.pth"
    ...
    def run_eval(self, loader, pred_file):
        ...
        #     for i, ath in enumerate(dlt_auc_ths):
