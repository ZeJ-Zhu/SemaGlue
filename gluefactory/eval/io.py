...
def parse_eval_args(benchmark, args, configs_path, default=None):
    ...
    if args.conf:#��������в����ṩ��conf
        conf_path = parse_config_path(args.conf, configs_path)
        custom_conf = OmegaConf.load(conf_path)#���������ļ����������custom_conf
        conf = extract_benchmark_conf(OmegaConf.merge(conf, custom_conf), benchmark)#��conf��custom_conf���
        args.tag = (
            args.tag if args.tag is not None else conf_path.name.replace(".yaml", "")#����������ṩ�˱�ǩ������ȥ��yaml
        )

    cli_conf = OmegaConf.from_cli(args.dotlist)#�������в����д������ö���
    conf = OmegaConf.merge(conf, cli_conf)#�ٴκϲ�
    conf.checkpoint = args.checkpoint if args.checkpoint else conf.get("checkpoint")#���δ�ṩʹ�������е�checkpoint
