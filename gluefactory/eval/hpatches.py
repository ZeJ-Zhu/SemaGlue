...
class HPatchesPipeline(EvalPipeline):
    ...
    def get_predictions(self, experiment_dir, model=None, overwrite=False):
        pred_file = experiment_dir / "predictions.h5"  #每个epoch都会得到一个predictions.h5  经过整个model的
        if not pred_file.exists() or overwrite:
            if model is None:
                model = load_model(self.conf.model, self.conf.checkpoint)
            #调用export_predictions来生成h5
            export_predictions(
    ...
    def run_eval(self, loader, pred_file):
        #pred_file指model得到的h5文件，predictions.h5
        ...
        #使用CacheLoader从pred_file中加载predictions.h5
        ...
        for i, data in enumerate(tqdm(loader)):
            ...
            #{'prec@1px': 0.22727273404598236, 'prec@3px': 0.9090909361839294, 'num_matches': 22, 'num_keypoints': 512.0,
            # 'H_error_dlt': 58.929847717285156, 'names': 'i_ajuntament/0.ppm', 'scenes': 'i_ajuntament'}
        ...
        summaries = {}
        #对于每个评估结果 v，如果其数据类型是数字，计算其中位数，并将结果存储在 summaries 字典中，以 "m" 作为前缀。
        for k, v in results.items():
            arr = np.array(v)#arr是一组data
            if not np.issubdtype(np.array(v).dtype, np.number):
                continue
            summaries[f"m{k}"] = round(np.median(arr), 3)#四舍五入到小数点后三位
        ...
        #生成homography_recall图形，显示DLT和RANSAC方法在不同阈值下的性能
        figures = {
