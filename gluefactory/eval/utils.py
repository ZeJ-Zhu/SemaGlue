...
def eval_matches_epipolar(data: dict, pred: dict) -> dict:
    ...
    # kp0, kp1 = pred["keypoints0"], pred["keypoints1"]
    # k0, k1, scores0 = pred["matches0"], pred["matches1"], pred["matching_scores0"]
    # pts0, pts1, scores = k0, k1, scores0
    ...
    correct = n_epi_err < 5e-4
    num_correct = torch.sum(correct).item()
    precision = torch.mean(correct.float()).item() if len(correct) > 0 else 0
...
def eval_homography_robust(data, pred, conf):
    ...
    return results #{'H_error_ransac': 5.4148759841918945, 'ransac_inl': 10.0, 'ransac_inl%': 0.45454545454545453}
...
def eval_homography_dlt(data, pred):
    ...
    kp0, kp1 = pred["keypoints0"], pred["keypoints1"]
    m0, scores0 = pred["matches0"], pred["matching_scores0"]
    #使用 get_matches_scores 函数从匹配信息中提取有效的匹配点
    pts0, pts1, scores = get_matches_scores(kp0, kp1, m0, scores0)
    ...
    try:
        ...
        #使用这些匹配点和得分通过 find_homography_dlt 函数估计单应性矩阵
        h_dlt = find_homography_dlt(pts0, pts1, scores)
        if H_gt.ndim == 2:
            h_dlt = h_dlt[0]
    except AssertionError:
        h_dlt = H_inf
    #计算估计的单应性矩阵与真实单应性矩阵之间的角点误差（corner error）
    error_dlt = homography_corner_error(h_dlt, H_gt, data["view0"]["image_size"])
    results["H_error_dlt"] = error_dlt.item()#58.929847717285156
    return results
