...
def load_experiment(exper, conf={}, get_last=False, ckpt=None):
    ...
    exper = Path(exper)
    if exper.suffix != ".tar":
        #根据get_last的值，选择加载最新的检查点还是最佳检查点
        if get_last:
...
# @TODO: also copy the respective module scripts (i.e. the code)  获取模型的状态字典（对于分布式训练，获取 model.module 的状态）。
# 创建一个字典 checkpoint，包含模型、优化器、学习率调度器、配置、训练周期、损失和评估结果等信息。
# 如果没有提供 cp_name，则根据当前的周期和迭代次数生成一个默认的文件名。
# 保存检查点文件到指定目录。
# 如果当前评估结果比之前的最好结果好，更新最好结果并保存为 checkpoint_best.tar。
def save_experiment(
    model,
    optimizer,
    lr_scheduler,
