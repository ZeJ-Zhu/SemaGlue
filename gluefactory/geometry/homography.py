...
#返回单应性矩阵 H、原始角点坐标 full、变换后的角点坐标 warped 和 patch 尺寸 patch_shape
#模拟图像中的视角变化、旋转和平移
def sample_homography_corners(
    ...
    #创建另一个矩形区域，这将是仿射变换后的区域
    pts2 = create_center_patch(patch_shape)
    scale = min_pts1 - full
    found_valid = False
    cnt = -1
    while not found_valid:
        #使用 numpy 中的 uniform 函数生成一个形状为 (4, 2) 的二维数组
    ...
    if n_angles > 0 and difficulty > 0:
        ...
        for idx in range(1, n_angles):
            #将旋转后的点云映射到归一化空间，shape是用于归一化的数组
            warped_points = rotated[idx] / np.array(shape)
            #检查映射后的点是否都在[0.0, 1.0]范围内
...
def homography_corner_error(T, T_gt, image_size):
    ...
    #计算估计的投影点 corners1 和真实的投影点 corners1_gt 之间的欧氏距离
