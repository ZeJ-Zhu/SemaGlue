...
class MetaModel(ABCMeta):
    def __prepare__(name, bases, **kwds):#创建一个包含基类默认配置的配置字典 total_conf
    ...
    def is_initialized(self):
        ...
        for name, w in self.named_children():
            if isinstance(w, BaseModel):
                ...
                if n_params != 0 and not self.are_weights_initialized:
                    print(f"Initialization failed at component: {name}")
