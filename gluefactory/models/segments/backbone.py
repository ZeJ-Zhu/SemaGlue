...
#这里将所有的config['norm_typ']替换为'sync_bn'
...
class StemConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bn_momentum=0.99):
        ...
        self.proj = nn.Sequential(
                                    ...
                                    NormLayer(out_channels//2, norm_type=default_config['norm_type']),
                                    nn.GELU(),
                                    nn.Conv2d(out_channels//2, out_channels,
                                                kernel_size=(3,3), stride=(2,2), padding=(1,1)),
                                    NormLayer(out_channels, norm_type=default_config['norm_type'])
                                )
...
class BlockFFN(nn.Module):
    def __init__(self, in_channels, out_channels, hid_channels, ls_init_val=1e-2, drop_path=0.):
        super().__init__()
        self.norm = NormLayer(in_channels, norm_type=default_config['norm_type'])
...
class BlockMSCA(nn.Module):
    def __init__(self, dim, ls_init_val=1e-2, drop_path=0.0):
        super().__init__()
        self.norm = NormLayer(dim, norm_type=default_config['norm_type'])
...
class MSCANet(nn.Module):
    def __init__(self, in_channnels=3, embed_dims=[32, 64, 160,256],
                 ffn_ratios=[4, 4, 4, 4], depths=[3,3,5,2], num_stages=4,
                 ls_init_val=1e-2, drop_path=0.0,norm_type=default_config['norm_type']):
        ...
        for i in range(num_stages):
            ...
            norm_layer = NormLayer(embed_dims[i], norm_type=norm_type)
            cur += depths[i]
