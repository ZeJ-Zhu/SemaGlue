...
#from mmengine.model import MODELS as MMCV_MODELS
...
from mmcv.cnn.bricks import DropPath
# from mmengine.model.utils.weight_init import (constant_init, normal_init,
#                                         trunc_normal_init)
#-----------------------------------
# 注册机制
class Registry:
...
def build_norm_layer(cfg, num_features, postfix=''):
    ...
    # 如果 cfg 是 DictConfig 对象，则将其转换为普通字典
...
# nn.DropPath = StochasticDepth
#//////////////////////////////////////////////////////////////////////////////////////////////////////////////////

# class NormLayer(nn.Module):
#     #def __init__(self, inChannels, norm_type=config['norm_typ']):
#     def __init__(self, inChannels, norm_type=default_config['norm_type']):
#         super().__init__()
#         self.inChannels = inChannels
#         self.norm_type = norm_type
#         if norm_type == 'batch_norm':
#             # print('Adding Batch Norm layer') # for testing
#             self.norm = nn.BatchNorm2d(inChannels, eps=1e-5, momentum=float(default_config['BN_MOM']))
...
class StemConv(BaseModule):
    def __init__(self, in_channels, out_channels, norm_cfg):
        ...
        self.proj = nn.Sequential(
            ...
            #NormLayer(out_channels//2, norm_type=norm_cfg),
            nn.GELU(),
            nn.Conv2d(out_channels // 2, out_channels,
                      kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),
            build_norm_layer(norm_cfg, out_channels)[1],
            #NormLayer(out_channels, norm_type=norm_cfg)
        )
...
class Block(BaseModule):
    def __init__(self,
        ...
        #self.norm1 = NormLayer(dim, norm_type=norm_cfg)
        self.attn = SpatialAttention(dim)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = build_norm_layer(norm_cfg, dim)[1]
        #self.norm2 = NormLayer(dim, norm_type=norm_cfg)
...
class OverlapPatchEmbed(BaseModule):
    ...
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768, norm_cfg=dict(type='SyncBN', requires_grad=True)):
        ...
        #self.norm = NormLayer(embed_dim, norm_type=norm_cfg)
...
#     if issubclass(class_type, nn.InstanceNorm1d) or issubclass(class_type, nn.InstanceNorm2d) or issubclass(class_type, nn.InstanceNorm3d):
#         return 'in'
#     elif issubclass(class_type, nn.BatchNorm1d) or issubclass(class_type, nn.BatchNorm2d) or issubclass(class_type, nn.BatchNorm3d):
#         return 'bn'
#     elif issubclass(class_type, nn.GroupNorm):
#         return 'gn'
#     elif issubclass(class_type, nn.LayerNorm):
...
#                        drop=drop)
#         layer_scale_init_value = 1e-2
#         self.layer_scale_1 = layer_scale_init_value * torch.ones((dim))
#         self.layer_scale_2 = layer_scale_init_value * torch.ones((dim))
