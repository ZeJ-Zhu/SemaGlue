...
class _MatrixDecomposition2DBase(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.S = config['MD_S']#MD_S表示分解后的基的数量
        self.D = config['MD_D']#MD_D表示基的维度,在NMF中，基是非负的
        self.R = config['MD_R']#MD_R表示系数的维度，在矩阵分解中，系数是基的线性组合的权重

        #self.train_steps = config['TRAIN_STEPS']#训练时执行的步数
        #self.eval_steps = config['EVAL_STEPS']#评估时执行的步数
        self.train_steps = 6
        self.eval_steps = 6
        
        self.inv_t = config['INV_T']#用于计算系数的softmax函数中的温度参数
        self.eta = config['Eta']#用于在线更新（online update)中的学习率参数

        self.rand_init = config['RAND_INIT']#一个布尔值，指示是否使用随机初始化基
        # print(30*'=')
...
class HamBurger(nn.Module):
    def __init__(self, inChannels, config):
        super().__init__()
        self.put_cheese = config['put_cheese']#是否添加奶酪层
        C = config["MD_D"]

        # add Relu at end as NMF works of non-negative only
        self.lower_bread = nn.Sequential(nn.Conv2d(inChannels, C, 1),#卷积核大小1*1
    ...
    #如果 ham 模块有 online_update 方法的话，会调用 ham 模块的 online_update 方法，传递 bases 参数
    def online_update(self, bases):
