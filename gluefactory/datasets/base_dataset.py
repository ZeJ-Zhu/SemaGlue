...
def collate(batch):
    #这是一个自定义的collate函数，用于在 PyTorch DataLoader 中处理批量数据。
    #collate函数的主要目的是将一个批次的样本组合成一个单一的张量或数据结构，以便能够传递给模型进行训练或推断。
...
class BaseDataset(metaclass=ABCMeta):
    ...
    def __init__(self, conf):
        ...
        if isinstance(conf, dict):#如果传入为字典，转换为omegaconf
            conf = OmegaConf.create(conf)
        self.conf = OmegaConf.merge(default_conf, conf)
        OmegaConf.set_readonly(self.conf, True)#只读
        logger.info(f"Creating dataset {self.__class__.__name__}")#Creating dataset HomographyDataset
        self._init(self.conf)

    @abstractmethod
    def _init(self, conf):#子类继承这个父类时，必须提供 _init 方法的具体实现，确保子类按照父类的要求进行必要的初始化工作
    ...
    def get_data_loader(self, split, shuffle=None, pinned=False, distributed=False):#根据指定的数据集拆分（"train"、"val" 或 "test"）返回一个数据加载器
        ...
        try:
            ...
            batch_size = self.conf.batch_size#3
