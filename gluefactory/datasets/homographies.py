...
artifacts.
"""#这是一个用于加载图像并应用单应性适应的数据集类主要配置项包括：
#data_dir: 数据集的顶层目录。
#image_dir: 存储图像的子目录。
#image_list: 可选，图像文件列表或文件路径。
#train_size 和 val_size: 训练和验证数据集的大小。
#homography: 单应性变换的配置，包括难度、平移、最大角度、角度数量、图像块形状等。
#photometric: 光度变换的配置，如亮度、对比度等。
#load_features: 控制是否加载图像的特征，包括是否进行缓存、缓存的参数等。
#此外，还包括用于下载 revisitop1m 数据集的函数 download_revisitop1m。
...
from ..models.segment_loader import Homo_Train_SegmentLoader, Homo_Val_SegmentLoader
...
def sample_homography(img, conf: dict, size: list):
    data = {}
    H, _, coords, _ = sample_homography_corners(img.shape[:2][::-1], **conf)#返回单应性矩阵和原图四个角点变换后的角点坐标
...
class HomographyDataset(BaseDataset):
    default_conf = {
        # image search
        "data_dir": "revisitop1m",  # the top-level directory数据集的顶层文件
        "image_dir": "jpg/",  # the subdirectory with the images存储图像的子目录
        "image_list": "revisitop1m.txt",  # optional: list or filename of list
        "glob": ["*.jpg", "*.png", "*.jpeg", "*.JPG", "*.PNG"],
        # splits
        "train_size": 100,
        "val_size": 10,
        "shuffle_seed": 0,  # or None to skip#0或者None表示不进行洗牌
        # image loading
        "grayscale": False,
        "triplet": False,
        "right_only": False,  # image0 is orig (rescaled), image1 is right
        "reseed": False,#是否重新生成随机种子
        "homography": {#几何变换的字典
            "difficulty": 0.8,
            "translation": 1.0,
            "max_angle": 60,
        ...
        "load_segments":{
            "do": False,
            **Homo_Train_SegmentLoader.default_conf,#Train和val一样
            "collate": False,
        }
    }

    def _init(self, conf):
        data_dir = Path(DATA_PATH) / Path(conf.data_dir)#conf.data_dir:revisitop1m
        if not data_dir.exists():
            ...
            logger.info("revisitop1m dataset is existing.")

        image_dir = data_dir / conf.image_dir#存储图像的目录，根据配置加载图像列表conf.image_dir:'jpg/'
        images = []
        if conf.image_list is None:#glob匹配图像文件，构建图像列表
        ...
        elif isinstance(conf.image_list, (str, Path)):#是字符串或path对象，直接读取该文件作为图像列表,进入这里
            image_list = data_dir / conf.image_list#PosixPath('/data1/suhzhang/dataset/Matcher/glue_data/revisitop1m/revisitop1m.txt')
        ...
        elif isinstance(conf.image_list, omegaconf.listconfig.ListConfig):#是omegaconf.listconfig.ListConfig对象，将其转换为python列表
    ...
    def download_revisitop1m(self):
        ...
        if tmp_dir.exists():  # The previous download failed.重新下载
            shutil.rmtree(tmp_dir)
        image_dir = tmp_dir / self.conf.image_dir#存储图像
        ...
        for n in tqdm(range(num_files), position=1):#迭代下载每个压缩文件
            ...
            tar_path.unlink()
        shutil.move(tmp_dir, data_dir)#从临时数据路径移动

    def get_dataset(self, split):
        return _Dataset(self.conf, self.images[split], split)#新建了个实例


class _Dataset(torch.utils.data.Dataset):
    def __init__(self, conf, image_names, split):
        self.conf = conf
        self.split = split#存储数据集的拆分
        self.image_names = np.array(image_names)
        self.image_dir = Path(DATA_PATH) / Path(conf.data_dir) / Path(conf.image_dir)

        aug_conf = conf.photometric
        aug_name = aug_conf.name#lg
        assert (
            aug_name in augmentations.keys()
        ), f'{aug_name} not in {" ".join(augmentations.keys())}'
        self.photo_augment = augmentations[aug_name](aug_conf)#存储图像的全局增强
        self.left_augment = (#存储图像左侧的增强
        ...
        if conf.load_segments.do:
            self.segment_train_loader = Homo_Train_SegmentLoader(conf.load_segments)
            self.segment_val_loader = Homo_Val_SegmentLoader(conf.load_segments)

    def _transform_keypoints(self, features, data):#用于对关键点进行变换，通过单应性变换、阈值处理、筛选最佳关键点等操作。
#这个方法主要处理特征的预处理，确保特征在模型输入之前经过适当的变换和筛选。potentially keep only the best ones
    ...
    # 从数据集中获取单个样本。根据索引加载图像，应用单应性变换、光度变换和图像增强，同时处理特征（如果需要加载）。
    # 最后返回一个字典，包含图像、单应性矩阵、特征等信息
    ...
    def _read_view(self, img, H_conf, ps, left=False):
        ...
        #     segments =  self.segment_loader({k: [v] for k, v in data.items()})
        #     data["segment"] = segments

        return data
    
    # 获取数据样本，包括左视图、右视图和单应性变换矩阵。如果配置中设置了生成三元组数据（triplet），还会生成第三个视图和相应的单应性变换矩阵。
    def getitem(self, idx):
        ...
        if self.conf.right_only:#只对右图进行变换
            left_conf["difficulty"] = 0.0

        data0 = self._read_view(img, left_conf, ps, left=True)
        # data0['segment'].pop('feature_map1', None)
        # value0 = data0['segment']['feature_map0']
        # data0['segment']['feature_map'] = value0
        # data0['segment'].pop('feature_map0')
        data1 = self._read_view(img, self.conf.homography, ps, left=False)
        # data1['segment'].pop('feature_map0', None)
        # value1 = data1['segment']['feature_map1']
        # data1['segment']['feature_map'] = value1
        # data1['segment'].pop('feature_map1')
        ...
        if self.conf.load_segments.do:
            ...
            data["segment"] = segments
            value0 = data['segment']['feature_map0']
            value1 = data['segment']['feature_map1']
            data = {key: value for key, value in data.items() if key != 'segment'}
            if 'segment' not in data['view0']:
                data['view0']['segment'] = {}
            if 'segment' not in data['view1']:
                data['view1']['segment'] = {}
            data['view0']['segment']['feature_map'] = value0
            data['view1']['segment']['feature_map'] = value1
...
def visualize(args):
    ...
    conf = OmegaConf.merge(conf, OmegaConf.from_cli(args.dotlist))#和默认conf合并
