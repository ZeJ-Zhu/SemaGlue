...
def sample_descriptors(keypoints, descriptors, s):#从输入的关键点和描述子中采样描述子
    b, c, h, w = descriptors.shape
    #从坐标以图像左上角为原点的坐标系转换为以图像中心为原点的坐标系(不确定)
...
class SuperPoint(BaseModel):
    ...
    checkpoint_url="https://gitee.com/SH_Z/SuperGluePretrainedNetwork/raw/master/models/weights/superpoint_v1.pth" #noqa:E501
    ...
    def _init(self, conf):
        ...
        if conf.has_descriptor:
         ...
         # Specify the destination path for downloading the checkpoint
        checkpoint_path = "/home/timer/.cache/torch/hub/checkpoints/superpoint_v1.pth"  # You can replace this with your desired local path
        #torch.hub.download_url_to_file(self.checkpoint_url, checkpoint_path, progress=True)
        state_dict = torch.load(checkpoint_path, map_location=torch.device('cpu'))#也许可以改gpu
    ...
    def _forward(self, data):
        ...
        if self.conf.sparse_outputs:#启用了稀疏输出
            ...
            if self.conf.force_num_keypoints:#走if
            ...
            if (len(keypoints) == 1) or self.conf.force_num_keypoints:
                ...
                if self.conf.legacy_sampling:#走if
                ...
                if self.conf.legacy_sampling:
                    ...
                    desc = [
                        ...
                        for k, d in zip(keypoints, c)
                    ]

            pred = {
                "keypoints": keypoints + 0.5,
                "keypoint_scores": scores,
                "descriptors": desc.transpose(-1, -2),#将张量的最后两个维度互换
            }
